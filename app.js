...
function renderHome() {
  ...
  const today = getTodayJapaneseDay();

  const todayPlans = data.plans.filter(
    plan => plan.day === today
  );
  ...
  container.innerHTML = todayPlans.map(plan => {
    const actualSeconds =
      getTodaySubjectSeconds(plan.subject);
